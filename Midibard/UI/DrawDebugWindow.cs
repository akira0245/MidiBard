...
#region Generator
    ...
    private static void DrawFontIconView()
    {
        if (ImGui.Begin("IconTest"))
        {
            ...
            if (ImGui.InputText("se", ref searchedstring, 100))
            {
                ...
                {
                    searched = glyphs.Where(i => i.name.ContainsIgnoreCase(searchedstring)).ToArray();
                }
                else
                {
                    searched = glyphs;
    ...
    static readonly (FontAwesomeIcon icon, string name)[] glyphs = 
        ...
        .Select(i => ((FontAwesomeIcon)i, ((FontAwesomeIcon)i).ToString())).ToArray();
